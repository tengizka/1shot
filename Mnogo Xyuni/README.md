...
Исходники и эта техническая инструкция находятся в `Mnogo Xyuni/`.
Все команды ниже выполняйте из этой папки:

```sh
cd "Mnogo Xyuni"
```

Пути ниже указаны относительно неё, кроме `.github/workflows/pages.yml`:
workflow остаётся в корне репозитория. При публикации из ветки корневой
`index.html` перенаправляет на `Mnogo%20Xyuni/public/`, `CNAME` сохраняет
домен `app.tengizka.fun`, а `.nojekyll` отключает обработку Jekyll.
Для публикации через GitHub Actions workflow загружает `Mnogo Xyuni/public`.

Адрес киберхауса 1shot: **улица Мира, 37**.
...
public/                     фронт мини-аппа (статика, GitHub Pages)
  ...
  config.js                 ⚙️ API_BASE, название/адрес, зоны (ZONES)
  map-data.js               ⚙️ карта зала: координаты машин, зоны, линии
  assets/logo.png           (опц.) ваш логотип — положить сюда; без него
                            используется неоновый CSS-вордмарк «1SHOT»
supabase/
...
Визуал — под бренд 1shot: чёрный фон, неоновый фиолет/розовый/синий/красный,
статусы: **свободен — неоново-зелёный, занят — красный, ваша бронь — розовый
с пульсом**, не в сети/ремонт — серый.

Два вида (табы):
- **Карта зала** — география компьютеров как в планировке клуба: VIP-зал
  (ряды 11-15, 21-25), основной зал (101-106, 201-208, 301-304, 401-402),
  PS5 (#1). Координаты — в `public/map-data.js` (x/y в % от карты),
  зоны-обводки и линии-разделители — там же. Добавили машину в Gizmo —
  допишите её в `machines: []`, и она появится на карте.
- **Список** — те же машины, сгруппированные по зонам.

Зоны определяются в `public/config.js` → `ZONES`:
1. по `groupIds` (id групп хостов из Gizmo — самый надёжный способ,
   впишите их из Gizmo Manager, тогда зоны не зависят от нумерации);
2. иначе по номеру машины (`numbers` / `ranges`);
3. `fallback: true` ловит остальных.

Логотип: положите PNG в `public/assets/logo.png` — он подхватится
автоматически; без файла показывается неоновый CSS-текст «1SHOT».
...
2. GitHub Pages: включите в Settings → Pages → **Source: GitHub Actions**
   (воркфлоу `.github/workflows/pages.yml` уже есть и сам деплоит `Mnogo Xyuni/public/`
...
npm test           # тесты (Node 22+, нативный TS): initData, gizmo, handlers
...
Пока API_BASE содержит YOUR_PROJECT_REF и приложение открыто вне Telegram,
включается явно помеченное демо: примерные статусы по планировке, выбор места,
длительности и локальная имитация бронирования/отмены. Запросов к API нет;
демоброни исчезают при перезагрузке. После настройки API требуется Telegram.

Локально: `python3 -m http.server 8000 --bind 0.0.0.0 --directory public`.
На узких экранах карту можно прокручивать горизонтально; для быстрого выбора
крупными кнопками есть вкладка «Список».
