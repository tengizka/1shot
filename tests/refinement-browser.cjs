...
(async()=>{
 ...
 try{
  ...
  await page.locator('#nav-profile').click();await page.waitForSelector('#request-password');const text=await page.locator('#ov-profile').innerText();assert.ok(!/Мои компьютеры|Выход подтверждён|Мини-апп|Сервер:/.test(text));assert.match(text,/version 1.4.1/);assert.ok(await page.locator('#request-password').evaluate(e=>e.classList.contains('btn-main')));
